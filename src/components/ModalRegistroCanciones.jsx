...
const ModalRegistroCanciones = ({ isVisible, onClose, onSongAdded }) => {
    ...
    useEffect(() => {
        const fetchAuthors = async () => {
            const response = await fetch('https://arsistemaweb.com/playlist2/back/api/autor/getAutor.php');
            const data = await response.json();
            setAuthors(data);
        };

        const fetchGeneros = async () => {
            const response = await fetch('https://arsistemaweb.com/playlist2/back/api/genero/getGenero.php');
            const data = await response.json();
            setGeneros(data);
        };
    ...
    const handleSubmit = async (e) => {
        e.preventDefault();
        const response = await fetch('https://arsistemaweb.com/playlist2/back/api/cancion/postCancion.php', {
            method: 'POST',
            headers: {
